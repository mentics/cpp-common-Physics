...
public:
	...
	virtual void posVel(const double atTime, vect3& pos, vect3& vel) const {
		double t = (atTime - startTime);
		vel = v0 + t * a0;
		pos = p0 + t * v0 + (0.5 * t * t) * a0;
	}

	virtual void posVelGrad(const double atTime, vect3& posGrad, vect3& velGrad) const {
		double t = (atTime - startTime);
		posGrad = v0 + t * a0;
		velGrad = a0;
	}
